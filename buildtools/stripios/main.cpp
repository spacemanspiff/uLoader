...
/* TODO - Use u32 instead of unsigned int */
...
unsigned int getbe32(void* pvoid)
{
	unsigned char* p = (unsigned char*)pvoid;
...
void putbe32(void* pvoid, unsigned int val)
{
	unsigned char* p = (unsigned char*)pvoid;
...
unsigned int strip=0;
	...
	unsigned int elfmagicword = getbe32(&header.ident0);
	...
	unsigned int phoff = getbe32(&header.phoff);
	unsigned short phnum = getbe16(&header.phnum);
	unsigned int memsz = 0, filesz = 0;
	unsigned int vaddr = 0, paddr = 0;
	...
	for(int i=0; i<phnum; i++)
	{
		...
		if (p == iosphentry)
		{
			unsigned int startoffset = phoff;
			unsigned int startvaddr = vaddr;
			unsigned int startpaddr = paddr;
	...
	for(int i=0; i<phnum; i++)
	{
		...
		unsigned int offset = getbe32(&p->offset);
		unsigned int filesz = getbe32(&p->filesz);

		if (filesz)
		{
			fseek(fin, offset, SEEK_SET);
            
			fprintf(stdout,"Writing segment 0x%08X to 0x%08lX (%d bytes) - %x %s\n", getbe32(&p->vaddr), ftell(fout), filesz,getbe32(&p->memsz),
