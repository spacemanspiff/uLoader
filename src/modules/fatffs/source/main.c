...
char * syscall_open_func( char *name, int mode)
{
	...
	if (name[0] == '#') {// for example #/title/xxxx : can be used to skip this function to open files
		...
		for (n = 0; sdio[n]; n++) 
			sys_name[n + 32] = sdio[n];
		sys_name[n + 32] = 0;
		
		name=(char *) sys_name + 32;
	} else if(!test_string(name)) {// redirect some routes
		int m;
		int ind=index_dev;
		
		if(flag_emu_mode & 1)
			if (!cmp_string(name, "/title/00010005") || 
			    !cmp_string(name, "/ticket/00010005")) 
				ind &= 1; // use sd:/nand or usb:/nand for DLC
		
		for (m = 0; dev_names[ind][m]; m++) 
			sys_name[m] = dev_names[ind][m];
		sys_name[m] = '/';
		m++;
...
s32 __FAT_Ioctlv(ipcmessage *message)
{
	...
	case IOCTL_FFS_MODE: {
		...
		if (diary_mode && usb_mounted < 0 && sd_mounted < 0)
			flag_emu_mode |= 128; // only capture the diary
...
static u32 __stm_out[0x08] ATTRIBUTE_ALIGN(32) = {
	0, 0, 0, 0, 0, 0, 0, 0
};
...
void release_event_handler(void)
{
	...
	if (stm_fd < 0)
		stm_fd = os_open("/dev/stm/immediate", 0);
	if (stm_event < 0 && stm_fd >= 0)
		stm_event = os_open("/dev/stm/eventhook", 0);
...
int thread_blink(void)
{
	...
	while (1) {
		...
		if ((blinker_off & 4) == 4) {
			os_restart_timer(timer_id, 1000*1000);
			continue;
		}
		...
		if (blinker_off & 128) {
			swi_mload_led_on(); 
			os_restart_timer(timer_id, 1000*5);
			blinker_off|=1;
		} else  {
			swi_mload_led_off();
			os_restart_timer(timer_id, 1000*1000);
...
int main(void)
{
	...
	/********************************************************************************************************************************************************/
	// led blink thread
	int blink_thread_id = os_thread_create((void *) thread_blink, 
					       NULL, 
					       &thread_stack[1024], 
					       1024, 
					       0x79, 
	...
	if (blink_thread_id >= 0)
		os_thread_continue(blink_thread_id);
	...
	while (1) {
		...
		case IOS_OPEN: {
			...
			memcpy(path_name, 
			       (char *) message->open.device, 
			       strlen((char *) message->open.device) + 1);
			devpath = path_name;
			...
			// redirected SD
			if (!strcmp(devpath, DEVICE_FAT"/dev/sdio"))  {// SD in game without interferences
				ret= IPC_EACCESS;
				break;
			} 
			...
			if(!strcmp(devpath, DEVICE_FAT"/dev/immediate")) {
				if (stm_fd < 0) 
					stm_fd = os_open("#/dev/stm/immediate", mode);
				
				if (stm_fd < 0) 
					ret = stm_fd; 
				else 
					ret= 0x888; // dev id for immediate
				break;
			}

			// fast command
			if (!strcmp(devpath, DEVICE_FAT":off")) {
				flag_emu = 0;
				ret= -1234;
				break;
			}
			if (!strcmp(devpath, DEVICE_FAT":on"))  {
				flag_emu = 1;
				ret = -1234;
				break;
			}
		...
		case IOS_READ: {
			...
			*/
			...
			if (ret < 0) {
				internal_debug_printf = 1;
				if (verbose_level) 
					debug_printf("Err FAT Read ret %i len i \n", ret, len);
				internal_debug_printf = 0;
			}
		...
		case IOS_SEEK: {
			...
			if(ret < 0) {
				internal_debug_printf = 1;
				if (verbose_level) 
					debug_printf("Err FAT Seek ret %i %u %u \n", ret, where, whence);
				internal_debug_printf = 0;
			}
			break;
		}
		...
		case IOS_IOCTLV: {
			...
			if (get_fat_fd(message->fd) >= 0) {
				...
				internal_debug_printf = 1;
				if (verbose_level) 
					debug_printf("eh! you cannot call ioctlv %x from here!\n", message->ioctlv.command);
				internal_debug_printf = 0;
				while (1) {
					swi_mload_led_on();
					Timer_Sleep(5000*1000);
		...
		case IOS_IOCTL:	{
			...
			if(message->fd == 0x888) {// /dev/stm/immediate descriptor
				...
				// Redirect to original ioctl
				ret = os_ioctl(stm_fd, cmd, message->ioctl.buffer_in,  
					                    message->ioctl.length_in,
			...
			if (cmd == FFS_IOCTL_GETFILESTATS) {
				...
				internal_debug_printf = 1;
				if (verbose_level) 
					debug_printf("eh! you cannot call ioctl 0x from here!\n", message->ioctl.command);
				internal_debug_printf = 0;
				while (1) {
					swi_mload_led_on();
					Timer_Sleep(5000*1000);
