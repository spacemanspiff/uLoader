...
int call_home_menu(struct discHdr *header, int function)
{
	...
	if (function == 1) {
		...
		if (delete_test(25, header->title)) {
			...
			disc_conf[2]='R';
			disc_conf[3]=0;
			disc_conf[9]=0; // break PNG signature
	...
	if (function == 2) {
		...
		draw_snow();
		Screen_flip(); 
		if (rename_game(header->title)) {
			global_RenameGame(header->id, header->title);
			WBFS_Close();
			return 2;
		} else 
			return 0;
	...
	if (function == 3) {
		...
		draw_snow();
		Screen_flip(); 
		if (delete_test(29, header->title)) {
			WBFS_RemoveGame(header->id);
			WBFS_Close();
			return 2;
		}
		else
		...
		for(n = 0; n < 6; n++) {
			...
			if (Draw_button2(30 + 48, 
					 ...
					 ((!header && !mode_disc && (m == 2 || m == 3 || m == 6 || m == 7 || m == 9 ))  
					  ...
					  || (is_fat && header && (header->version & 2) && (m == 6 || m == 9))) ? -1 : 0))
				select_game_bar = m + 1;
	//
		}
		...
		if (wmote_datas != NULL) {
			...
			if (wmote_datas->ir.valid) {
				wiimote_ir();
			} else if (wmote_datas->exp.type == WPAD_EXP_GUITARHERO3) {
				wiimote_guitar();
			} else if (wmote_datas->exp.type == WPAD_EXP_CLASSIC) {
				wiimote_classic();
			} else {
				px=-100;
				py=-100;
			...
			if (new_pad & WPAD_BUTTON_A) {
				if (select_game_bar == 50) {
					index -= 6;
					if (index < 0)
						index=6;
					snd_fx_yes();
				}
				if (select_game_bar == 51) {
					index += 6;
					if (index>6) 
						index = 0;
					snd_fx_yes();
...
int menu_format()
{
	...
	char type[5][9] = {
		"Primary ", "Extended", "WBFS Pri", "WBFS Ext","Unknown "
	};
...
re_init:
	...
	while (1) {
		...
		/*for (n = 0; n < num_partitions; n++) {
		          if (type[partition_type[n].type>>8][0]>1) {
		                  it_have_wbfs = 1;
                                  break;
                          }
		  }*/
		...
		if (level >= 100) {
			...
			if (level >= 1000) {
				...
				if ((partition_type[m].type>>8)==4) {
					s_printf("Unpartitioned Disc (%s) LBA: %.10u LEN: %.2fGB", 
						 &type[partition_type[m].type>>8][0],
						 partition_type[m].lba, 
						 ((float)partition_type[m].len * (float)sector_size) / (1024*1024*1024.0));
				} else {
					s_printf("Partition %2.2i -> %2.2xh (%s) LBA: %.10u LEN: %.2fGB",
						 m+1,
						 partition_type[m].type & 255, 
						 &type[partition_type[m].type>>8][0],
						 partition_type[m].lba, 
						 ((float)partition_type[m].len * (float)sector_size) / (1024*1024*1024.0));
				}
				...
				if (Draw_button(600 - 32 - strlen(&letrero[idioma][18][0])*8, ylev + 108*4 - 64, &letrero[idioma][18][0])) 
					select_game_bar = 61;
			}
		}
		...
		if(level == 0) {
			for(n = 0; n < 6; n++) {
				...
				if (m >= num_partitions) {
					...
					if (px >= PX - 32 && 
					    ...
					    py >= PY - 8 &&
					    py < PY + 32) {
						if ((partition_type[m].type >> 8) > 1 &&
						    (partition_type[m].type >> 8) != 4)
							DrawRoundFillBox(PX - 40, PY - 12, 60*8 + 80, 48, 0, 0xff00cf00);
						else 
							DrawRoundFillBox(PX - 40, PY - 12, 60*8 + 80, 48, 0, 0xffcfcfcf);
					
						DrawRoundBox(PX - 40, PY - 12, 60*8 + 80, 48, 0, 5, 0xfff08000);
						select_game_bar = 100 + m;
					} else {
						if ((partition_type[m].type >> 8) > 1)
							DrawRoundFillBox(PX - 32, PY - 8, 60*8 + 64, 40, 0, 0xff00cf00);
						else 
							DrawRoundFillBox(PX - 32, PY - 8, 60*8 + 64, 40, 0, 0xffcfcfcf);
			...
			if (Draw_button(36, ylev + 108*4 - 64, 
					&letrero[idioma][0][0]))
				select_game_bar = 1;
			
			if (Draw_button(600 - 32 - strlen(&letrero[idioma][0][0])*8, 
					ylev+108*4-64,
					&letrero[idioma][0][0])) 
				select_game_bar = 1;
		} // level==0
		...
		if (level < 1000) {
			if (wmote_datas != NULL) {
				...
				if ((new_pad &  WPAD_BUTTON_PLUS) && level == 0) {
					current_partition += 6;
					if (current_partition > (num_partitions+6))
						current_partition -= 6;
				}
				...
				if (new_pad & WPAD_BUTTON_A) {
					...
					if (select_game_bar == 50) { //page -6
						snd_fx_yes();
						current_partition -= 6;
						if (current_partition < 0)
							current_partition = 0;
					}
					if (select_game_bar == 51) { // page +6
						snd_fx_yes();
						current_partition += 6;
						if(current_partition > (num_partitions + 6))
							current_partition -= 6;
					...
					if(select_game_bar >= 100 && select_game_bar < 150) {// select partition to format
						level = select_game_bar;
						snd_fx_yes();
					}
					
					if (select_game_bar == 60) { // Yes
						level+=900;
						snd_fx_yes();
					}
					if (select_game_bar == 61) { // No
						snd_fx_no();
						level=0;
					}
				}
		...
		if (level >= 1000){
			if (level < 2000) { // error
				if (WBFS_Format(partition_type[level-1000].lba, 
						partition_type[level-1000].len) < 0) 
					level = 2048;
				else 
					level = 2048+128; // ok
			}
...
void add_game()
{
...

	{
		...
		while(1) {
			...
			if (new_pad & (WPAD_GUITAR_HERO_3_BUTTON_GREEN | WPAD_BUTTON_A)) {
				snd_fx_yes();
				break;
			}
		}
...
int delete_test(int ind, char *name)
{
	...
	while (1) {
		...
		if (Draw_button(600 - 32 - strlen(&letrero[idioma][18][0])*8, 
				ylev + 108*4 - 64, &letrero[idioma][18][0])) 
			select_game_bar=61;
	...
	while (1) {
		...
		if(mode == 1) {
			...
			for(n = 0; n < 4; n++) {
				...
				PY = my_y + 6 + 8;
				color = INK0;
				s_printf("%c", parental_str[n]==0 ? ' ' : parental_str[n]);
		...
		if (mode==0) {
			...
			for(n = 0; n < 8; n++) {
				...
				if (config_file.game_log[n].id[0]) {
					...
					s_printf("%c%c%c%c%c%c ",
						 config_file.game_log[n].id[0],
						 config_file.game_log[n].id[1],
						 config_file.game_log[n].id[2],
						 config_file.game_log[n].id[3],
						 config_file.game_log[n].id[4],
						 config_file.game_log[n].id[5]);

					memset(title_str, 32, 22);
					title_str[22] = 0;

					if(gameList && gameCnt > 0) {
						for(l = 0; l < gameCnt; l++) {
							if (!strncmp( (void *) gameList[l].id, 
								      (void *) config_file.game_log[n].id, 6)) {
								for (k = 0; k < 22; k++) 
									if(gameList[l].title[k] == 0)
										break; 
									else 
										title_str[k] = (gameList[l].title[k]<32) ? ' ': gameList[l].title[k];
							}
						}
					}

					color=0xffff1000; 
					s_printf("%s ",title_str);
					color = INK0;

					s_printf("%c%c-%s-%c%c%c%c %c%c:%c%c",
						 (config_file.game_log[n].bcd_time[0] >> 4) + 48, 
						 (config_file.game_log[n].bcd_time[0] & 15) + 48,
						 &month[idioma][((config_file.game_log[n].bcd_time[1] >> 4)*10 + 
								 (config_file.game_log[n].bcd_time[1] & 15) - 1) % 12][0],
						 (config_file.game_log[n].bcd_time[2] >> 4) + 48, 
						 (config_file.game_log[n].bcd_time[2] & 15) + 48,
						 (config_file.game_log[n].bcd_time[3] >> 4) + 48, 
						 (config_file.game_log[n].bcd_time[3] & 15) + 48,
						 (config_file.game_log[n].bcd_time[4] >> 4) + 48,
						 (config_file.game_log[n].bcd_time[4] & 15) + 48,
						 (config_file.game_log[n].bcd_time[5] >> 4) + 48,
						 (config_file.game_log[n].bcd_time[5] & 15) + 48);
				} else {
					autocenter=1;
					s_printf("< Empty >");
					autocenter=0;
			...
			if (Draw_button(36, ylev + 108*4 - 64, 
					&letrero[idioma][0][0]))
				select_game_bar = 61;
			if (Draw_button(600 - 32 - strlen(&letrero[idioma][46][0])*8, 
					ylev + 108*4 - 64, 
					&letrero[idioma][46][0])) 
				select_game_bar = 70;
		}
		...
		if (mode == 2) {
			...
			for (n = 0;n < 5; n++) {
				...
				PY = my_y + 6 + 8;
				color = INK0;
				s_printf("%c", n==4 ? '0' : parental_str[n]);
				DrawRoundBox(my_x, my_y, 48, 48, 0, 4, 0xa0000000);
			}
		...
		if (wmote_datas != NULL) {
			...
			if (new_pad & WPAD_BUTTON_A) {
				if (select_game_bar == 60) { // Yes
					...
					snd_fx_yes(); 
					for ( n = 0; n < 4; n++) 
						config_file.parental[n] = parental_str[n]-48;
					save_cfg(mode_disc != 0);
					return;
				}
				if (select_game_bar == 61) { // No
					for (n = 0; n < 4; n++) 
				...
				if (select_game_bar >= 20000 &&
				    select_game_bar < 20010) {
					for (n = 0; n < 4; n++) 
						if (parental_str[n] == 0)
							break;
					if (n < 4) {
...
int rename_game(char *old_title)
{
	...
	while(1) {
		...
		color = INK1; 
		bkcolor = 0x20000000;
		PX = (SCR_WIDTH - sx*len_str) / 2;
		PY = 56;
		for (n = 0; n < len_str; n++) {
			PX = (SCR_WIDTH - sx*len_str)/2 + n*sx; 
			PY=56;
			if (n == cursor && (frames2 & 8) == 0) {
				bkcolor = 0xa000ff00;
				s_printf(" ");
				bkcolor=0x20000000;
				PX -= sx; 
		...
		if (Draw_button(x_temp + 16, ylev + 108*4 - 64, &letrero[idioma][48][0]))
			select_game_bar = 65;
			...
			if (new_pad & WPAD_BUTTON_A) {
				if (select_game_bar == 60) {// OK
					if (len_str < 2) {
						...
						snd_fx_yes();
						title[63] = 0;
						memcpy(old_title,title,64);
						return 1;
					}
				}
				if (select_game_bar == 61) { // discard
					draw_snow();
					Screen_flip();
					snd_fx_no();
					return 0;
				...
				if (select_game_bar == 100) {
					shift ^= 4;
					snd_fx_yes();
				}
				
				if (select_game_bar == 101) {
					cursor--;
					if (cursor<0) {
						cursor=0;
						snd_fx_no();
				...
				if (select_game_bar >= 1024) {
					if (len_str > 62) 
						...
						for (n = 62; n >= cursor; n--)
							title[n+1] = title[n];
						title[cursor] = select_game_bar - 1024; 
						title[63] = 0;
						len_str = strlen(title);
						snd_fx_yes();
						cursor++;
...
// temp dol alternative table for DVD mode
u8 alt_dol_disc_tab[32768]  __attribute__ ((aligned (32))); 
...
void menu_alternativedol(u8 *id)
{
	...
	for (indx = 0; indx < 1024; indx++) {
		if (dol_infop[indx].id[0] == 0 && last_indx == 1023)
			last_indx = indx;
		...
		if (mode == 0) {
			...
			if(indx < 1024) {
				if (dol_infop[indx].offset != 0 && dol_infop[indx].size != 0)
					if (Draw_button(x_temp + 16, ylev + 108*4 - 64, &letrero[idioma][49][0]))
						select_game_bar = 61;	
				//PX= 20; PY+=32;
				//s_printf("%llx %i",dol_infop[indx].offset, dol_infop[indx].size);
			}

			if (Draw_button(600 - 32 - strlen(&letrero[idioma][50][0])*8, ylev + 108*4 - 64, &letrero[idioma][50][0])) 
				select_game_bar = 62;
		} else if(mode==1) {
				...
				if (file != NULL) {
					...
					if ((l1+l2) > 64) {
						...
						if (l2 <= c2) {
							...
							memcpy(&buff[k], (char *) file->name, c);
							buff[k + c] = 0;
							strcat(buff,"...");
							c = c2 - c - 3;
					...
					if (Draw_button2(30+16, ylev+32-8+64*m, buff, 0)) 
						select_game_bar = 1024 + n;
					file = file->next;
				} else {
			...
			if (Draw_button(600 - 32 - strlen(&letrero[idioma][0][0])*8, ylev + 108*4 - 64, 
					&letrero[idioma][0][0])) 
				select_game_bar=60;
		} else if(mode == 2) {
			PX = 0;
		...
		if (mode < 128) {
			...
			if (wmote_datas != NULL) {
				...
				if (mode == 1) {
					...
					if (new_pad & WPAD_BUTTON_PLUS) {
						current_dol += 5;
						if (current_dol > (max_dol/5)*5)
							current_dol = (max_dol/5)*5;
						snd_fx_yes();
					}
				}
				...
				if(new_pad & WPAD_BUTTON_A) {
					...
					if (select_game_bar >= 1024) {
						...
						if (indx >= 1024) {
							if (last_indx == 1023)
								memcpy((char *) &dol_infop[0], 
								       ((char *) &dol_infop[0]) + 32, 32768 - 32);
							indx = last_indx;
						}
						memset((void *) &dol_infop[indx], 0, 32);
...
u32 stringcompare(char *s1, char *s2)
{
	...
	while (true) {
		...
		if (s1[index] != '?' && 
		    s2[index] != '?' && 
		    toupper((u8)s1[index]) != toupper((u8)s2[index])) {
			return index;
		}
		index++;	
	}
...
void wdm_alternativedol(u8 *id)
{
	...
	while (file) {
		...
		for (n = 0; n < dolmenubuffer[0].count; n++) {
			...
			if (!strncmp(dolmenubuffer[n+1].dolname, "main", 4))
				continue;
...
int uloader_update()
{
	...
	while (1) {
		...
		letter_size(16,32);
		if (Draw_button2(30+48, ylev+32+64*3-80, "                    Download uLoader                    ",0))
			select_game_bar=1;

		if (Draw_button2(30+48, ylev+32+64*3,    "              Download Alternative uLoader              ",0)) 
			select_game_bar=2;

		if (Draw_button2(30+48, ylev+32+64*3+80, "                          Exit                          ",0)) 
			select_game_bar=3;
	...
	if (ret == 0 && temp_buf != NULL) {
		if (!strncmp((char *) temp_buf, 
			     "<!DOCTYPE HTML", 14)  /*strstr((char *) temp_buf, "Not Found")*/) {
			down_frame = -1;
		...
		while (temp_buf[n] > 32 && n < temp_size && n < 4) {
			cad[n] = temp_buf[n];
			n++;
		}
		while (temp_buf[n] < 32 && n < temp_size) 
			n++;
            
		if (strncmp((void *) uloader_version, (void *) cad,4) == 0)
			update = 2;
		else if (strncmp((void *) uloader_version, (void *) cad,4) < 0)
			update = 1;
	} else {
		down_frame = -1;
	...
	if (ret == 0 && temp_buf != NULL) {
		...
		if (sd_ok && temp_size != 0) {
			...
			if(!fp) {
				down_frame = -1;
				down_mess  = "Error Creating boot.dol in the SD";
				snd_fx_no();
				sleep(4);
				goto exit;
			}
				...
				if (r != temp_size) {
					down_frame = -1;
					down_mess  = "Error Writing boot.dol in the SD";
					snd_fx_no();
					sleep(4);
					goto exit;
				}
	...
	while (n < temp_size2) {
		...
		m = 0;
		while (temp_buf2[n] > 32 && n < temp_size2) {
			name[m] = temp_buf2[n];
			n++;
			m++;
		}
		...
		if(ret == 0 && temp_buf != NULL) {
			...
			if (sd_ok && temp_size != 0) {
				...
				if (!fp) {
					...
					down_frame=-1;
					sprintf((char *) cadena,"Error Writing %s in the SD", name);
					down_mess=(char *) cadena;
					snd_fx_no();
					sleep(4);
					goto exit;
			...
			if (mi_letrero == 0) {
				...
				if (new_pad & WPAD_BUTTON_A) {
					...
					if (select_game_bar == 110) {
						...
						Screen_flip();
						happy_new_year(-1);
					} else if(select_game_bar >= 100 && select_game_bar < 115) {
						snd_fx_yes();
						temp_hacks[select_game_bar-100] ^= 1;
					}
				}
			}
...
void get_covers()
{
	...
	for(n = 0; n <= gameCnt; n++) {
		...
		for (m = 0;m < gameCnt; m++) {
			...
			case 1:
			case 3:
				DrawFillBox(20 + m*sizex, SCR_HEIGHT/2 - 8 , sizex2, 16, 0, 0xff00cf00);
				break;
			case 2:
				DrawFillBox(20 + m*sizex, SCR_HEIGHT/2 - 8 , sizex2, 16, 0, 0xff0000ff);
				break;
			default:
				DrawFillBox(20 + m*sizex, SCR_HEIGHT/2 - 8 , sizex2, 16, 0, 0x30800000);
				break;
			}
		}
		...
		*/
		...
		if (!(disc_conf[0]=='H' && 
					...
					if (!strncmp((char *) header->id, get_name_from_UTF8(&files[m].name[0]), 6)) {
						...
						if (fp) {
							...
							if (!temp_buf) {
								...
								fseek(fp, 0, SEEK_SET);
								if (fread(temp_buf,1, temp_size ,fp) == temp_size)
									ret = 0;
			...
			sprintf(url, "http://www.wiiboxart.com/%s/%c%c%c%c%c%c.png", 
				&region[(header->id[3]=='E')+(header->id[3]=='J')*2][0], 
				header->id[0], header->id[1], header->id[2], 
				header->id[3], header->id[4], header->id[5]);
			...
			sprintf(url, "http://www.muntrue.nl/covers/%s/160/225/boxart/%c%c%c%c%c%c.png", 
				&region2[(header->id[3]=='E')+(header->id[3]=='J')*2][0], 
				header->id[0], header->id[1], header->id[2], 
				header->id[3], header->id[4], header->id[5]);
			...
			if (ret == 0) {
#if 0
				sprintf(url, "sd:/caratulas/%c%c%c%c%c%c.png", 
					header->id[0], header->id[1], header->id[2], 
					header->id[3], header->id[4], header->id[5]);
				...
				if (temp_size>4 && temp_size<(200 * 1024 - 8)) {
					...
					if(texture_png) {
						...
						disc_conf[1] = 'D'; 
						disc_conf[2] = 'R';
						disc_conf[3] = ((temp_size+8+1023)/1024)-1;
	...
	while (1) {
			...
			if(flag_auto) {
				for (n = 0; n < nfiles; n++) {
					if (!files[n].is_directory)
						if (!strncmp((char *) header->id,get_name_from_UTF8(&files[n].name[0]), 6)) {
							signal = 1;
							posfile = n;
							flag_auto = 0;
							break;
				...
				if (flag_auto) {
					for (n = 0;n < nfiles; n++) {
						if (!files[n].is_directory)
							if (!strncmp((char *) header->id,get_name_from_UTF8(&files[n].name[0]), 4)) {
								signal=1;
								posfile=n;
								flag_auto=0;
								break;
		...
		if(wmote_datas!=NULL) {
			...
			if(wmote_datas->exp.type==WPAD_EXP_GUITARHERO3) {
				...
				if (new_pad & WPAD_GUITAR_HERO_3_BUTTON_STRUM_UP) 
					new_pad |= WPAD_BUTTON_UP;
				if (old_pad & WPAD_GUITAR_HERO_3_BUTTON_STRUM_UP) 
					old_pad |= WPAD_BUTTON_UP;

				if (new_pad & WPAD_GUITAR_HERO_3_BUTTON_STRUM_DOWN) 
					new_pad |= WPAD_BUTTON_DOWN;
				if (old_pad & WPAD_GUITAR_HERO_3_BUTTON_STRUM_DOWN) 
			...
			if (nfiles > 0) 
				if (new_pad & WPAD_BUTTON_A) {
					if (!files[posfile].is_directory) {
						if (is_ext(files[posfile].name,".png") && 
						    (mode==0 || mode==100)) {
							if(texture_png != NULL && mem_png != NULL) {
								...
								if (mode==0) 
									...
									disc_conf[1] = 'D'; 
									disc_conf[2] = 'R';
									disc_conf[3] = ((len_png + 8 + 1023)/1024) - 1;
						...
						snd_fx_yes();
						// cambio de device
						if (files[posfile].name[0]=='.' && 
						    files[posfile].name[1]=='[') {
							if (files[posfile].name[2]=='U') {
								...
								posfile = 0;
								flag_auto = 1;
							} else if(files[posfile].name[2]=='S') {
								current_device = 0;
							...
							read_list_file((void *) &files[posfile].name[0], flag);
							posfile=0;
							flag_auto=1;
						}
					}
			...
			if (mode == 0) {
				if (!(old_pad  & (WPAD_BUTTON_UP | WPAD_BUTTON_DOWN)))
					f = 0;
				...
				if (new_pad & (WPAD_BUTTON_HOME | WPAD_BUTTON_1 | WPAD_BUTTON_2)) 
					break;
