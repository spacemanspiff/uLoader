	...
	*nv = 1;
	...
	for (n=0; n<8; n++) {
		if ((b[n]>='0' &&  b[n]<='9') || 
		    ...
		    (b[n]>='a' &&  b[n]<='f'))   {
			v[4+(n>>1)]<<=4; 
			v[4+(n>>1)]|=(b[n])-48-7*(b[n]>='A')-41*(b[n]>='a');
		} else 
			return -1; // error en numero
	}
...
void set_cheats_list(u8 *id)
{
	...
	for (n=0; n<1024; n++) {
		if (cheat_file.cheats[n].magic!=0xae && free<0) 
			free=n;

		if (cheat_file.cheats[n].magic==0xae && 
		    !strncmp((char *) cheat_file.cheats[n].id, 
			     (char *) id, 
			     cheats_for_wiiware ? 4 : 6)) {
			for (m=0; m<25; m++) 
				cheat_file.cheats[n].sel[m]=(data_cheats[m].apply!=0);
			return;
		}
	}
	...
	for (m=0; m<25; m++) 
		cheat_file.cheats[free].sel[m]=(data_cheats[m].apply!=0);
}
	...
	for (n=0; n<1024; n++) {
		if (cheat_file.cheats[n].magic==0xae && 
		    !strncmp((char *) cheat_file.cheats[n].id, 
			     ...
			     cheats_for_wiiware ? 4 : 6))   	{
			for (m=0;m<25;m++) 
				data_cheats[m].apply = (cheat_file.cheats[n].sel[m]==1);
			return;
		}
	}
}
	...
	for(n=0; n<MAX_LIST_CHEATS; n++) {
		if (data_cheats[n].title && data_cheats[n].apply) {
			if(f==0)
				m+=8;
			f=1;
			memcpy(temp_buff_cheats+m,
			       data_cheats[n].values,
			       data_cheats[n].len_values);
...
int load_cheats(u8 *discid)
{
	...
	actual_cheat = 0;
	num_list_cheats = 0;
	memcpy(&file_cheats[10],
	       (char *) discid, 
	       6);
	len_cheats = 0;

	txt_cheats = 1;
	memset(data_cheats, 0, 
	       sizeof(struct _data_cheats)*MAX_LIST_CHEATS);
	fp = NULL;
	if (sd_ok) {
		sprintf(file_cheats,
			"sd:/codes/%c%c%c%c%c%c.txt", 
			discid[0], discid[1], discid[2], 
			discid[3], discid[4], discid[5]);
    
		fp = fopen(file_cheats, "rb");
		if(!fp)	{
			sprintf(file_cheats,
				"sd:/codes/%c%c%c%c.txt", 
	...
	if (txt_cheats) {
		...
		while(1) {
			...
			if (fgets(buff_rcheat,256,fp)) {
				...
				int ret;
				int ndatas=0;
				while (buff_rcheat[g]!=0 && 
				       buff_rcheat[g]!='\n' && 
				       buff_rcheat[g]!='\r') 
					g++;
				buff_rcheat[g] = 0;

				ret=gettype_line((u8 *) buff_rcheat, 
						 data_readed, 
						 &ndatas);
				...
				case 0: // get ID
					if (ret==1) {
						if (strncmp((char *) discid, 
							    (char *) buff_rcheat, 
							    cheats_for_wiiware ? 4 : 6)!=0)
							force_exit = 1; // error! código no coincide
						mode++;
					}
					break;
				...
				case 2: // get entry name
					if (ret!=0) {
						...
						memcpy (temp_sub_head, 
							buff_rcheat, 
							(g>31) ? 31 : g);
						temp_sub_head[(g>31) ? 31 : g] = 0;
						...
						memset(buff_cheats+m, ' ', 63); // rellena de espacios
						buff_cheats[m+63]=0;
							
							
						memcpy(buff_cheats+m+(63-(g+sub))/2,
						       sub_head,sub);// centra nombre
	    
						if(sub>0)
							buff_cheats[m+(63-(g+sub))/2+sub-1]='>';
	    
						memcpy(buff_cheats+m+(63-(g+sub))/2+sub,
						       buff_rcheat,g);// centra nombre
					...
					if (ret==0 || ret==1) {
						if(mode==4) {	
							...
							if(data_cheats[n].title) {
								memcpy(sub_head, temp_sub_head, 31);
								sub_head[31] = 0;
								sub = strlen((char *) sub_head); 
								if(sub<31) {
									sub_head[sub]   = '>';
