...
bool compare_videomodes(GXRModeObj* mode1, GXRModeObj* mode2)
{
	...
	if (mode1->viTVMode != mode2->viTVMode || 
...
#if 0
void mem_patch(void *buffer, u32 len, u8 *s, int s_size, 
	       int p_skip, u8 *p, int p_size)
{
	int n;
...
int compare_hex_str(u8 *buff, const u8 *hex_str)
{
	...
	while(*hex_str != 0) {
		...
		if (*hex_str >= 'A' && *hex_str <= 'F') 
			dat = (10+*hex_str-'A') << 4;
		else if (*hex_str >= 'a' && *hex_str <= 'f') 
			dat = (10 + *hex_str - 'a') << 4;
		else if (*hex_str >= '0' && *hex_str <= '9') 
			dat = (*hex_str - '0') << 4;
		else if(*hex_str == 'x' || *hex_str <= 'X') 
			dat = *buff & 0xf0;
		else {
			return 0;
		}
...
void patch_hex_str(u8 *buffer, u32 len, u32 step, 
		   const u8 *search, int skip_patch, 
		   const u8 *patch, int counter )
{
	u32 cnt;
...
void __Patch_CoverRegister(void *buffer, u32 len)
{
   ...
   const u8 newcode[] = "4800000C"; /*"5460F7FF4082000C546007FF"*/ 
...
*/
	...
	if(title_dol) {
		...
		if (global_mount & 3) {
			if (load_fatffs_module(NULL) < 0) {
				cabecera2( "Fail Loading FAT FFS Module!!!");
				sleep(4);
				return 17;
			}
		}
		...
		*/
		...
		if (nand_mode & 3) 
			enable_ffs( ((nand_mode-1) & 1) | ((nand_mode>>1) & 6) | 8 | (64*(dont_use_diary!=0)) | 128 );
...
int load_disc(u8 *discid)
{
	...
	if (is_fat) {
		...
		if ((global_mount & 3) || dont_use_diary) {
			if (load_fatffs_module(NULL) < 0) 
				return 17;
		} else if (shadow_mload) {// special: disables ffs and only works with syscall filter fat device (disebled when you opens "NO_MLOAD")
			disable_ffs_patch();
			if (load_fatffs_module(NULL) < 0) 
				return 17;
		}
	...
	if(is_fat) {// FAT device
		...
		if (WDVD_SetUSBMode((u8 *) "_DEV", 0)) 
			return 1;
	} else if(discid[6]==2) { // DVD USB
		if (WDVD_SetUSBMode((u8 *) "_DVD", multi_ciso[ciso_sel].lba) != 0)
			return 1;
	} else if (discid[6] != 0) {
		if (WDVD_SetUSBMode(NULL, multi_ciso[ciso_sel].lba) != 0) 
			return 1;
	} else {
		if (cios == 249 && current_partition != 0) 
			return 101;
		if (WDVD_SetUSBMode(discid, current_partition) != 0)
			return 1;
	}
	...
	*/
	if ((nand_mode & 3) || dont_use_diary )
		enable_ffs( ((nand_mode-1) & 1) | ((nand_mode>>1) & 6) | 8 | (64*(dont_use_diary!=0)) | 128 | (256* ((nand_mode & 16)!=0)));
	...
	*(vu32 *)0x800000F0 = 0x01800000;       // Simulated Memory Size
	...
	if ((strncmp((char *) discid, "R3XE6U", 6)==0) || 
	    (strncmp((char *) discid, "R3XP6V", 6)==0))   {
	...
	if (WDVD_OpenPartition((u64) Partition_Info.Offset, 0, 0, 0, Tmd_Buffer) != 0)
        ...
        if (IS_VALID_SIGNATURE(Certs) 	&& 
	    IS_VALID_SIGNATURE(Tmd) 	&& 
	    IS_VALID_SIGNATURE(Ticket)  &&  
	    C_Length  > 0               &&
	    MD_Length > 0               && 
	    T_Length > 0                    ) {
	...
	*/
	...
	while (Load(&Address, &Section_Size, &Partition_Offset)) {
		...
		if (i>=3) 
			wipregisteroffset((u32)Address, Section_Size);
