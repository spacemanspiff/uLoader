...
int CWIIDisc_io_read (unsigned char  *ptr, 
		      size_t size, 
		      struct image_file *image, u64 offset)
{
	...
	if (image->d) {
		...
		if (WDVD_UnencryptedRead(temp_ptr, size2, offset & ~3) < 0)
			size = 0;
...
int CWIIDisc_io_open(struct image_file *image, 
		     wbfs_disc_t *d)
{
#if 0
	if (filename[1] != '-') {
		...
		if (image->d) {
			...
			for (i = 0; i < count; i++) {
				...
				if (!wbfs_get_disc_info(image->d->p, i, b, 0x100, &size)) {
					if (strcmp((const char*) b,
						   (const char*) image->d->header->disc_header_copy) == 0)
...
int CWIIDisc_decrypt_block (struct image_file *image, 
			    u32 part, 
			    u32 block)
{
        ...
        if (CWIIDisc_io_read(image->parts[part].dec_buffer, 
			     0x8000, 
			     image,
			     image->parts[part].offset + 
			         image->parts[part].data_offset + (u64)(0x8000) * (u64)(block)) != 0x8000) {
                return -1;
        }
...
size_t CWIIDisc_io_read_part (unsigned char *ptr, 
			      size_t size, 
			      ...
			      u64 offset)
{
        ...
        while (size) {
                ...
                memcpy (dst, 
			image->parts[part].cache + cache_offset,
                        cache_size);
                dst  += cache_size;
                size -= cache_size;
                cache_offset = 0;
			...
			char **partition)
{
	...
	//char buffer[255];
		...
		MarkAsUsedDC(image->parts[part].offset + image->parts[part].data_offset, 
			     offset, size, image->parts[part].is_encrypted);
...
//		HTREEITEM hPartitionBin;
	...
	//  _fstat (image->fp, &image->st);
		...
		if (image->parts[i].type != PART_DATA)
			continue;
...
bool CWIIDisc::SaveDecryptedFile(const char *csDestinationFilename,  struct image_file *image, u32 part, u64 nFileOffset, u64 nFileSize, BOOL bOverrideEncrypt)
{
	...
	fOut = fopen(csDestinationFilename, "wb");
        ...
        if ((!image->parts[part].is_encrypted)|| (1 == bOverrideEncrypt)) {
