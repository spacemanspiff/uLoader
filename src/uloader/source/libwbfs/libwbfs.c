		    ...
		    int reset)
{
        ...
        u8 *ptr;
	u8 *tmp_buffer = wbfs_ioalloc(hd_sector_size);
        u8 part_table[16*4];
	int l;
        ...
        //find wbfs partition
	/*
	  if (tmp_buffer[0x1bc] != 0x00 || 
	  tmp_buffer[0x1bd] != 0x00 || 
	  tmp_buffer[0x1fe] != 0x55 || 
	  tmp_buffer[0x1ff] != 0xaa) 
	  wbfs_memset(part_table,0,16*4);
	*/
	if (tmp_buffer[0x1fe] != 0x55 || 
	    tmp_buffer[0x1ff] != 0xaa || 
	    !strncmp((void *) &tmp_buffer[0x03], "NTFS", 4) || 
	...
	l = 0;
		...
		if (ptr[4] == 0x0f) {  // Extended partition type
			...
			for (n = 0; n < 8; n++) { // max 8 logic partitions (i think it is sufficient!)
				...
				ret = read_hdsector(callback_data,
						    part_lba + next_lba2,
						    1,
						    tmp_buffer);
				if (ret) {
				...
				part_lba2 = part_lba + next_lba2 + 
					read_le32_unaligned(tmp_buffer + 0x1C6);
				...
				if (head->magic == wbfs_htonl(WBFS_MAGIC)) {
					...
					if (l > 0 && WBFS_part_list[l - 1] == WBFS_part_list[l])
						WBFS_part_list[l] = 0xFFFFFFFF;
					else 
						l++; 
	...
	if (WBFS_part_list[0] == 0xFFFFFFFF) 
		return 0;
...
int wbfs_disc_read(wbfs_disc_t*d,
		   u32 offset, 
		   u8 *data, 
		   u32 len)
{
		...
		if (nlb > 64) // read protection
			nlb = 64; 
                ...
                err = p->read_hdsector(p->callback_data,
				       p->part_lba + (iwlba<<iwlba_shift) + lba, 
				       nlb, 
				       ptr);
                if(err)
                        return err;
...
int wbfs_wiiscrub_read_disc(wbfs_disc_t*d, 
			    u64 offset, 
			    s32 len, 
			    char *data)
{
	...
	copy_buffer = (u8*)wbfs_ioalloc(p->wbfs_sec_sz); // ->mal
        ...
        for(i = sector; i< p->n_wbfs_sec_per_disc; i++) {
			...
			*/
			...
			if (i == 0) {
				if (copy_buffer[1024] == 'H' && 
				    copy_buffer[1025] == 'D' && 
				    copy_buffer[1026] == 'R')   {
					wbfs_memset(&copy_buffer[1024], 
						    0, 
						    copy_buffer[1027]*1024 + 1024);
				}
			}
			...
			if (offset) {
				memcpy(dst, 
				       copy_buffer + offset,
				       size > (p->wbfs_sec_sz-offset) ? (p->wbfs_sec_sz-offset):size);
				...
				memcpy(dst + dst_pos,
				       copy_buffer,
				       size < p->wbfs_sec_sz ? size:p->wbfs_sec_sz);
			...
			if (!size) 
				break;
                        /*
			write_dst_wii_sector(callback_data, 
					     i * dst_wbs_nlb, 
					     dst_wbs_nlb,
					     copy_buffer);
			j++;
...
// hermes
// offset is pointing 32bit words to address the whole dvd, although len is in bytes
int wbfs_disc_write(wbfs_disc_t*d,
		    u32 offset, 
		    u8 *data, 
		    u32 len)
{
...
u32 wbfs_get_disc_info(wbfs_t*p, 
		       u32 index,
		       u8 *header,
		       int header_size,
		       u32 *size)//size in 32 bit
{
        u32 i;
	u32 count=0;
        int disc_info_sz_lba = p->disc_info_sz >> p->hd_sec_sz_s;
                        ...
                        if (count++ == index) {
						 ...
						 p->tmp_buffer);
							 ...
							 header2);
                                        ...
                                        u32 sec_used = wbfs_sector_used(p,
									(wbfs_disc_info_t *) header2);
                                        wbfs_iofree(header2);
...
static void free_block(wbfs_t *p,
		       int bl)
{
        int i = (bl - 1) / 32;
        int j = (bl - 1) & 31;
...
u32 wbfs_estimate_disc(wbfs_t *p, 
		       ...
		       partition_selector_t sel)
{
	...
	read_src_wii_disc(callback_data, 
			  0, 
			  0x100, 
			  info->disc_header_copy);
...
// data extraction
u32 wbfs_extract_disc(wbfs_disc_t*d, 
		      rw_sector_callback_t write_dst_wii_sector,
		      void *callback_data,
		      progress_callback_t spinner)
{
        wbfs_t *p = d->p;
        ...
        for (i = 0; i< p->n_wbfs_sec_per_disc; i++) {
				...
				ERROR("Reading Error");
                        ...
                        if (write_dst_wii_sector(callback_data, 
						 i*dst_wbs_nlb, 
						 dst_wbs_nlb, 
						 copy_buffer) < 0) 
				ERROR("Writing Error");
...
int read_wiidisc_wbfsdisc(void*fp,
			  u32 offset,
			  u32 count,
			  void*iobuf)
{
        return wbfs_disc_read((wbfs_disc_t*)fp, 
			      offset, 
			      iobuf, 
			      count);
}
...
int wbfs_extract_file(wbfs_disc_t*d, 
		      char *path, 
		      void **data)
{
        wiidisc_t *wd = 0;
