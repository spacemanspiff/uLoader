...
char *name;
	...
	if(!is_fat) WBFS_RenameGame(discid, new_name);

	name=get_fat_name(discid);
	if(!name) return 0;

	if(strstr(name, "#uloader.dat")) return 0;
	...
	if(fp)
		...
		new_name[63]=0;
		if(fwrite(new_name, 1, 64 ,fp)!=64) goto err;

		memcpy((char *) fat_disc[fat_index].title, (char *) new_name, 64);
...
char *name;
	...
	if(!is_fat)  WBFS_SetBCA(discid, bca);
	...
	if(fp)
		...
		if(fwrite(bca, 1, 64 ,fp)!=64) goto err;
...
s32 global_LoadDolInfo(void *data)
{
	...
	if(!is_fat) return WBFS_LoadDolInfo(data);
...
{
	if(!is_fat) return WBFS_SaveDolInfo(data);
...
char *name;
	...
	if(!is_fat) return WBFS_GetProfileDatas(discid, buff);
	...
	if(fp)
		...
		if(fread(buff, 1, 1024 ,fp)!=1024) { memset((void *) buff,0,1024); goto err; }
		if(buff[0]=='H' && buff[1]=='D' && buff[2]=='R' && buff[3]!=0)
			...
			if(((u32) buff[3])<201) 
				{
				if(fread(buff+1024, 1, ((u32) buff[3])*1024 ,fp)!=((u32) buff[3])*1024){ memset((void *) buff,0,1024); goto err;}
				}
...
char *name;
	...
	if(!is_fat) return WBFS_SetProfileDatas(discid, buff);
	...
	if(fp)
		...
		if(fwrite(buff, 1, 1024 ,fp)!=1024) goto err;

		if(buff[0]=='H' && buff[1]=='D' && buff[2]=='R' && buff[3]!=0 && ((u32) buff[3])<201)
			{
			if(!strstr(name, "#uloader.dat"))
				fseek(fp, 32768+2048, SEEK_SET);
			else
			...
			if(fwrite(buff+1024, 1, ((u32) buff[3])*1024 ,fp)!=((u32) buff[3])*1024) goto err;
			}
		fclose(fp);
		return 1;
		}
...
if(is_fat)
	...
	if(mode==0) // from WBFS
		...
		if(sd_ok)
			{
			fp=fopen("sd:/apps/uloader/uloader.cfg","rb"); // lee el fichero de configuracion
			}
		if(ud_ok && !fp)
			{
...
FILE *fp=NULL;
	...
	if(mode==0)
		...
		{
		if(!sd_ok && !ud_ok) return;

		config_file.magic=0xcacafea2;
		
		if(sd_ok)
			{
			fp=fopen("sd:/apps/uloader/uloader.cfg","wb"); // escribe el fichero de configuracion
			}
		if(ud_ok && !fp)
			{
			fp=fopen("ud:/uloader.cfg","wb"); // escribe el fichero de configuracion
			}
...
char filename[256];
	...
	while(n<256)
		...
		if(!(filestat.st_mode & S_IFDIR)) 
			{
				if(!(is_ext(namefile,".ciso") )) continue;
				...
				if(fp!=0)
					{
					if(fread(p,1, 4 ,fp)!=4) flag=0;

					if(flag && !(p[0]=='C' && p[1]=='I' && p[2]=='S' && p[3]=='O')) flag=0; // isn't CISO file
					
					if(flag)
						{
						fseek(fp, 0x8000, SEEK_SET);

						if(fread(p,1, 0x60 ,fp)!=0x60) flag=0;
						}
					...
					if(flag)
						...
						for(m=0;m<n;m++)
							{
							if(!strncmp((char *) fat_disc[m].id, (char *) p, 6)) break; // test ID
							}
						
						/*m=n;*/

						memcpy((char *) fat_disc[m].id, (char *) p, 6); // ID
	...
	while(n<256)
		...
		if((filestat.st_mode & S_IFDIR)) 
			...
			int Tmd_len;
				...
				if(!strncmp(namefile,"48",2) || !strncmp(namefile,"55",2)) continue;
				...
				if(FAT_read_file(&fat_games[n][0], &Tmd, &Tmd_len)>=0)
					...
					for(m=0;m<n;m++)
						{
						if(!strncmp((char *) fat_disc[m].id, (char *) fat_disc[n].id, 6)) break; // test ID
						}
						...
						for(i=0;i<p_tmd->num_contents;i++)
							...
							if(Content[i].index==0) 
								...
								sprintf(appnamefile, "%s%s/content/%08x.app", nand_path, namefile,  Content[i].cid);
						...
						if(!fp)
							...
							if(!(disc_conf[9]=='P' && disc_conf[10]=='N' && disc_conf[11]=='G') 
								...
								sprintf(filename, "%s%s/content/%08x.app", nand_path, namefile, *((u32 *)fat_disc[m].unused2));
								...
								if(FAT_read_file(filename, &app, &size_app)==0)
									...
									if(update_info)
										...
										if(fp)
											...
											if(fwrite(disc_conf, 1, 1024 ,fp)!=1024) goto err;

											if(disc_conf[0]=='H' && disc_conf[1]=='D' && disc_conf[2]=='R' && disc_conf[3]!=0 && ((u32) disc_conf[3])<201)
												{
												
												fseek(fp, 1024, SEEK_SET);

												if(fwrite(disc_conf+1024, 1, ((u32) disc_conf[3])*1024 ,fp)!=((u32) disc_conf[3])*1024) goto err;
												}
											err:
											fclose(fp);
											}
						...
						if(n==m) n++;
							//else sprintf(&fat_games[m][0], "%s%s", device, namefile);
							else  sprintf(&fat_games[n][0], "%s%s/content/#uloader.dat", nand_path, namefile);
...
int n;
	...
	for(n=0;n<size;n++)
	{
		if(!memcmp((void *) &p[n],"ULO_CFG",8) && p[n+8]==0x12 && p[n+9]==0x34 && p[n+10]==0x00 && p[n+11]==0x01)
		{
		return &p[n];
		}
	}
