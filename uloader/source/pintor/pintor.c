...
// funcion evento que dibuja todos los elementos de la pantalla virtual al LCD
void DrawScreen() 
{
	int n, m;
	unsigned *p, c, c2;
...
// rutina usada para dibujar caracteres (coordenadas de 8x8)
void v_putchar( unsigned x, unsigned y, unsigned color, unsigned char ch) 
{
   int 	i,j,v;
   unsigned char	*font;
...
unsigned PALETA[10] = { 
	0x0, 0xff0000ff, 0xff00ff00, 0xff00ffff, 0xffff0000, 0xffff00ff, 0xffffff00, 0xffffffff, 0xff000000, 1
};

// dibuja un UDG
void SetUDG(unsigned x,unsigned y,unsigned char *punt) 
{
	int n, m, v;
	unsigned col;
	if (x >= ANCHO || y >= ALTO) 
...
int map_screen_rings[10] = {
	66, 11, 10, 10, 12, 13, 0, 0, 0, 0
};
...
void copyMAP()
{
	...
	switch(ACTMAP) {
	case 0:
		COLORFONDO = 0x80800000;
		PALETA[2] = 0x80ff8000;
		break;
	case 2:
		COLORFONDO = 0x80004020;
		PALETA[2] = 0x8000ff80;
		break;
	case 3:
		COLORFONDO = 0x80404040;
		PALETA[2] = 0x808000ff;
		break;
	case 4:
		COLORFONDO = 0x80404000;
		PALETA[2] = 0x8000ff00;
		break;
	case 5: 
		COLORFONDO = 0x80402040;
		PALETA[2] = 0x80ff00ff;
		break;
	default:
		COLORFONDO = 0x80804000;
		PALETA[2]=0x80ff8000;
	}
	...
	for(n = 0; n < 22; n++) {
		for(m = 0;m < 32; m++) {
			...
			if (c == ' ')
				...
				c = 65;
			else if (c == 'B' && nbichos > 1) 
				c = 33;
			else if (c == 'C' && nbichos > 2) 
				c = 17;
			else if (c == 'D' && nbichos > 3)
				c = 9;
			else 
				c = 1;
			map_screen[n][m] = c;
...
void DrawMAP() // dibuja el mapa y actualiza variables
{
	...
	if (rings >= map_screen_rings[ACTMAP] || 
	    piok || 
	    pausa || 
	    ACTMAP == 0) 
		count2 = 20; // evita efectos de sonido
	else
		rings+=AnalizeMAP();
	/*
	if(rings>map_screen_rings[ACTMAP]) 
		return ; // si se han comppletado los anillos retorna
	*/
	...
	for (n = 0; n < 22; n++) {
		...
		for (m = 0; m < 32; m++) {
			...
			if (c & 128) {
				...
				SetUDG(m, n, (unsigned char *) &gdu_sprite[4*(protadir==1 || protadir==4)][0][0]);
				c = (c & 0xfc) | 130;
				map_screen[n][m] = c;
			}
			// veo bichos!!
			if (c & 64) {
				...
				if (count2 == 0) 
					message_camina[0]=(m - 16) + (m==16);
				SetUDG(m,n,(unsigned char *) &gdu_sprite[6+paso+2*(bichodir[0]==1 || bichodir[0]==4)][0][0]);
			}
			if(c & 32) {
				if ((c & 128) && piok == 0) {
					piok = 16384 + (n << 5) + m;
				...
				if (count2 == 0) 
					message_camina[1] = (m - 16) + (m==16);
				SetUDG(m, n, (unsigned char *) &gdu_sprite[6+paso+2*(bichodir[1]==1 || bichodir[1]==4)][0][0]);
			}
			if (c & 16) {
				if ((c & 128) && piok == 0) {
					piok = 16384 + (n << 5) + m;
				...
				if (count2 == 0) message_camina[2] = (m - 16) + (m == 16);
				SetUDG(m,n,(unsigned char *) &gdu_sprite[6+paso+2*(bichodir[2] == 1 || bichodir[2] == 4)][0][0]);
			}
			if (c & 8) {
				if ((c & 128) && piok == 0) {
					piok = 16384 + (n << 5) + m;
				...
				if (count2 == 0) 
					message_camina[3] = (m - 16) + (m == 16);
				SetUDG(m, n, (unsigned char *) &gdu_sprite[6+paso+2*(bichodir[3] == 1 || bichodir[3] == 4)][0][0]);
			}
		}
	}
	...
	if (ACTMAP != 0) { // si no es pantalla 0
		...
		for (n = 0; n < vidas; n++) 
			SetUDG(29 + n, 23, (unsigned char *) &gdu_sprite[0]);
	} else {
...
// llamada por analizeMAP(), me gustan los nombres rarus
int subanalizeMAP(int x,int y) 
{
	int n, m;
	...
	if ((map_screen[y  ][x + 1] & 3) != 2 || 
	    (map_screen[y+1][x    ] & 3) != 2) 
		return 0;
	...
	for(m = x + 1;m < 32; m++) {
		if ((map_screen[y][m] & 3) == 2) {
			if ((map_screen[y + 1][m] & 3) == 1)
				break;
			if ((map_screen[y + 1][m] & 3) == 2) {
				xx2 = m;
				break;
			}
		} else
	...
	for(m=y+1;m<22;m++) {
		if ((map_screen[m][x] & 3) == 2) {
			if ((map_screen[m][x+1] & 3) == 1)
				break; 
			if ((map_screen[m][x+1] & 3) == 2) {
				yy2 = m;
				break;
			}
		} else
	...
	for (m = xx1; m <= xx2; m++) {
		if ((map_screen[yy2][m] & 3)!= 2)
			break;
	}

	if (m <= xx2) // no esta completo el circulo
		return 0; 

	for(m = yy1; m <= yy2; m++) {
		if ((map_screen[m][xx2] & 3) != 2)
			break;
	}
	if (m <= yy2) // no esta completo el circulo
		return 0; 
...
// rutina que sirve para que el programa se entere de lo que pasa en el mapa (busca anillos)
int AnalizeMAP() 
{
	int numrec = 0;
	int n, m;
		...
		for(m = 0; m < 32; m++) {
			...
			if (c & mask) {// detectado bicho
				do {
					Wait_Event();
					if (bichodir[nbi] == 0) {
						bichodir[nbi] = (rand() & 1) + 1;
						message_effect2 = 1;
					}

					if (bichodir[nbi] == 1 || bichodir[nbi] == 2) {
						if (n > 0)
							if (map_screen[n-1][m] & 3) 
								if((rand() & 15) < 4) {
									bichodir[nbi] = 4;
									message_effect2 = 1;
								}
						...
						{
							if (m > 0) 
								if (map_screen[n][m-1] & 3) 
									if((rand() & 15) < 4) {
										bichodir[nbi] = 1;
										message_effect2 = 1;
									} 
							if (m < 31) if (map_screen[n][m+1] & 3) 
									    if ((rand() & 15) < 4) {
										    bichodir[nbi] = 2;
										    message_effect2 = 1;
									    }
					...
					if (bichodir[nbi] & 1) {
						if (m > 0) 
							if (map_screen[n][m-1] & 3) {
								map_screen[n][m] &= mask2;
								map_screen[n][m-1] |= mask;
								testmove++;
							}
					} else if(bichodir[nbi] & 2) {
						if (m < 31)
							if (map_screen[n][m+1] & 3) {
								map_screen[n][m] &= mask2;
								map_screen[n][m+1] |= mask;
					...
					} else if(bichodir[nbi] & 4) {
						if (n > 0) 
							if (map_screen[n-1][m] & 3) {
								map_screen[n][m]&=mask2;
								map_screen[n-1][m]|=mask;
								testmove++;
							}
					} else if(bichodir[nbi] & 8) {
						if (n < 21) 
							if (map_screen[n+1][m] & 3) {
								map_screen[n][m] &= mask2;
								map_screen[n+1][m] |= mask;
			...
			if (c & 128) { // detectado protagonista
				...
				if ((pad & PAD_LEFT) && protadir != 1) {
					if (m > 0) 
						if (map_screen[n][m-1] & 3) 
							protadir = 1;
				} else if((pad & PAD_RIGHT) && protadir != 2) {
					if (m < 31) 
						if (map_screen[n][m+1] & 3)
							protadir = /*(protadir  & 12) |*/ 2;
				} else if((pad & PAD_UP) && protadir != 4) {
					if (n>0) 
						if (map_screen[n-1][m] & 3)
							protadir=/*(protadir  & 3) |*/ 4;
				} else if((pad & PAD_DOWN) && protadir!=8) {
					if (n<21) 
						if (map_screen[n+1][m] & 3) 
							protadir=/*(protadir  & 3) |*/ 8;
				}
				...
				if (protadir & 1) {
					if (m > 0) 
						if (map_screen[n][m-1] & 3) {
							map_screen[n][m] &= 127;
							map_screen[n][m-1] = (map_screen[n][m-1] & 0xfc) | 130;
						}
				} else if (protadir & 2) {
					if (m < 31) 
						if (map_screen[n][m+1] & 3) {
					...
					if (n > 0) 
						if (map_screen[n-1][m] & 3) {
							map_screen[n][m] &= 127;
							map_screen[n-1][m] = (map_screen[n-1][m] & 0xfc) | 130;
						}
				} else if (protadir & 8) {
					if(n < 21) 
						if (map_screen[n+1][m] & 3) {
	...
	*/
	...
	do {
		...
		do {
			...
			while(1==1) {// bucle principal donde se desarrolla toda la accion :O
				...
				if (!time4_sound && !pausa) {
					if(message_effect1) {
						mod_sound = 0;
						message_effect1 = 0;
						set_sonido( 127 << 8, 1200, 125, 0, 1);
					} else if (message_effect2) {
						mod_sound = 1;
						message_effect2 = 0; 
						set_sonido((63 << 8), 300, 75, 125, 1);
					} else /*if(message_effect1)*/ {
						mod_sound = 1;
						message_effect1 = 0; 
						set_sonido((32 << 8), 150, 60, 125, 1);
					}
				}

				if (rings >= map_screen_rings[ACTMAP]) // pasa a sigueinte pantalla
					break; 
				...
				if (PADEVENT) { // lectura del PAD por interrupciones :D
					...
					if (wmote_datas != NULL && wmote_datas->exp.type == WPAD_EXP_CLASSIC) {
 						...
 						if (temp_pad & WPAD_CLASSIC_BUTTON_UP) 
							temp_pad |= WPAD_BUTTON_RIGHT;

						if (temp_pad & WPAD_CLASSIC_BUTTON_DOWN) 
							temp_pad |= WPAD_BUTTON_LEFT;
		
						if (temp_pad & WPAD_CLASSIC_BUTTON_LEFT) 
							temp_pad |= WPAD_BUTTON_UP;

						if (temp_pad & WPAD_CLASSIC_BUTTON_RIGHT)
							temp_pad |= WPAD_BUTTON_DOWN;
					...
					if (ACTMAP == 0) {
						...
						// ajusta bichos
						if (new_pad & (WPAD_BUTTON_UP | WPAD_CLASSIC_BUTTON_UP)) {
							nbichos--; 
							if (nbichos<2) 
								nbichos = 2;
							copyMAP();
						}
						if (new_pad & (WPAD_BUTTON_DOWN | WPAD_CLASSIC_BUTTON_DOWN)) {
							nbichos++; 
							if(nbichos > 4) 
								nbichos = 4;
							copyMAP();
			...
			n = 0;
			if (piok == 0) { // rutina fin de pantalla
				message_scroll = 1;
				set_sonido(127 << 8, 200, 100, 100, -1);
				mod_sound = 0;
				//n=100;
				...
				while (n < 26) {
					...
					if (count20ms > 2) {
						n++;
						set_sonido(127 << 8, 1900 - 300*n / 26, 50, 0, 1);
