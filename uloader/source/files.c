...
#define BRK_STR 17
	...
	while (name[m]!=0) {
		if (name[m] == '/') // elimina la parte de directorio
			n = m;
		m++;
	}
	n++;
...
#define BRK_STR 17
	...
	m = 0;
	n = -1;
	while (name[m] != 0) {  // elimina la parte de directorio
		if (name[m] == '/') 
			n = m;
		m++;
	}
...
void read_list_file(unsigned char *dirname, int flag)
{
	...
	if (!dirname) {
		...
		if (dirname[n-1] == '/') { // esto solo pasa si es directorio raiz
			...
			sprintf(swap, "%s/", dirname);
			if (swap[n-1] == '.') { // para volver al directorio anterior
				n--;
				while (swap[n] != '/')
					n--;
				n--;
	...
	if (fd != NULL) {
		while (nfiles < MAX_ENTRY) {
			...
			if (filestat.st_mode & S_IFDIR) {
				...
				/*
				  if (flag == 0) {
					  if (!(is_ext(files[nfiles].name,".mp3") || 
						is_ext(files[nfiles].name,".ogg") || 
						is_ext(files[nfiles].name,".m3u") ))
						  continue;
					  if (!is_ext(files[nfiles].name,".m3u")) 
						  test_music_file(nfiles, 0);
				  }
				  if (flag == 1)
					  if (!(is_ext(files[nfiles].name,".jpg") || 
						is_ext(files[nfiles].name,".jpeg"))) 
						  continue;
				*/
				if (flag == 0)
					if (!(is_ext(files[nfiles].name,".png") )) 
						continue;
	...
	for (n=0; n < (nfiles); n++) 
		for (m = n+1; m < nfiles; m++) {
			...
			if (files[m].is_directory == 1 && 
			    files[n].is_directory == 0)
				f = 1;
			if(f == 0) 
				if (files[m].is_directory == files[n].is_directory) 
					if (strcmp(files[m].name,files[n].name) < 0)
						f = 1;
			if (f) {
				f = 0;
				swap_file = files[m];
