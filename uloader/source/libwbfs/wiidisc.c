...
static void disc_read(wiidisc_t *d,
		      u32 offset,
		      u8 *data, 
		      u32 len)
{
...
static void partition_raw_read(wiidisc_t *d,
			       u32 offset, 
			       u8 *data, 
			       u32 len)
{
...
static void partition_read_block(wiidisc_t *d,
				 u32 blockno, 
				 u8 *block)
{
        u8*raw = d->tmp_buffer;
...
static void partition_read(wiidisc_t *d,
			   u32 offset, 
			   u8 *data, 
			   u32 len,
			   int fake)
		  ...
		  u32 i)
{
	...
	if (fst[12*i]) {
		...
		offset = _be32(fst + 12*i + 4);
                if(d->extract_pathname && 
		   strcmp(name, d->extract_pathname) == 0) {
...
static void do_files(wiidisc_t*d)
{
	...
	apl_size = 0x20 + _be32(apl_header + 0x14) + _be32(apl_header + 0x18);
        ...
        partition_read(d, apl_offset, 
		       0, apl_size, 1);

        partition_read(d, dol_offset, 
		       0,(fst_offset - dol_offset) << 2, 1);
...
static void do_partition(wiidisc_t *d)
{
	...
	partition_raw_read(d, 0x000 >> 2, tik, 0x2a4);
...
static int test_parition_skip(u32 partition_type,
...
wiidisc_t *wd_open_disc(read_wiidisc_callback_t read,
			void *fp)
{
...
u8 * wd_extract_file(wiidisc_t *d, 
		     partition_selector_t partition_type, 
		     char *pathname)
{
        u8 *retval = 0;
...
void wd_build_disc_usage(wiidisc_t *d, 
...
void wd_fix_partition_table(wiidisc_t *d,
			    partition_selector_t selector, 
			    u8* partition_table)
{
